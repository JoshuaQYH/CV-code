...
int Canny::readGrayImage(string inputFileName)
{
	try
	{
		SrcImage.load_bmp(inputFileName.c_str());
	}
...
void Canny::makeGaussianKernel(float sigma, float ** kernel, int * windowsize)
{
	...
	*windowsize = 1 + 2 * ceil(2.5 * sigma);  // ？？？？？？？？？？？？？？？？？？？？？？？ 咋来的
	center = (*windowsize) / 2;
...
功能： 删除图像中较短长度小于20的边缘，边缘长度定义为像素点的个数
	   过程是，对某一边缘进行广度搜索，统计长度，若小于等于20就将这些边缘点置为NOTEDGE.
	   达到剔除短边缘的目的。
******************************************************************/
void Canny::cutShortEdge(CImg<short int>* edge, CImg<short int>* resultEdgeImage)
{
	...
	CImg<short int> tmp(cols, rows, 1, 1, 0);
...
*/
void Canny::canny(string inputFileName) {
	cout << "正在检测图像边缘..." << endl;
	/*1. 读取图像，并转化为灰度图*/
	...
	/*8.  剔除短边缘*/
	cutShortEdge(&cutShortEdgeImage,&resultEdgeImage);
